...
class GWaveNetEncoder(nn.Module):
    def __init__(self,
        ...
        # logit_init = torch.logit(torch.tensor(0.1))
        # self.logit_crit_enc = torch.nn.Parameter(
        #     torch.full((286,), logit_init)
        # )

        logit_init = torch.logit(torch.tensor(0.1))
        self.logit_crit_enc = torch.nn.Parameter(logit_init.clone())
    


    def forward(self, input, adj, Edge_crit, mask_node=True):
    # def forward(self, input, adj, Edge_crit, Edge_noncrit_mask_path, mask_node=True):

        Edge_noncrit = (adj * (Edge_crit == 0).float() != 0).float()
        Edge_noncrit.fill_diagonal_(1.0)
        # Edge_fused = Edge_crit + torch.sigmoid(self.logit_crit_enc).view(-1, 1) * Edge_noncrit

        # Edge_noncrit_mask = torch.load(Edge_noncrit_mask_path, map_location="cpu")
        # self.Edge_noncrit_mask = Edge_noncrit_mask.to(adj.device)
        # Edge_noncrit = Edge_noncrit * self.Edge_noncrit_mask


        # Edge_fused = Edge_crit + torch.sigmoid(self.logit_crit_enc) * Edge_noncrit

        adj = [(Edge_noncrit / Edge_noncrit.sum(dim=-1, keepdim=True)), (Edge_noncrit.t() / Edge_noncrit.t().sum(dim=-1, keepdim=True))]
...
class GWaveNetDecoder(nn.Module):
    def __init__(self,
        ...
        # logit_init = torch.logit(torch.tensor(0.1))
        # self.logit_crit_dec = torch.nn.Parameter(
        #     torch.full((286,), logit_init)
        # )

        logit_init = torch.logit(torch.tensor(0.1))
        self.logit_crit_dec = torch.nn.Parameter(logit_init.clone())

    def forward(self, input, adj, Edge_crit):
        ...
        # # Edge_fused = Edge_crit + torch.sigmoid(self.logit_crit_dec).view(-1, 1) * Edge_noncrit

        # Edge_fused = Edge_crit + torch.sigmoid(self.logit_crit_dec) * Edge_noncrit
