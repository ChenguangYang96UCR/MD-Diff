...
class GWaveNetModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        # Edge_noncrit_mask_path = os.path.join(self.save_dir, f"Edge_noncrit_mask_N{num_nodes}.pt")
        # self.Edge_noncrit_mask_path = Edge_noncrit_mask_path
        # Edge_noncrit = (self.adj * (self.Edge_crit == 0).float() != 0).float()

        # if os.path.exists(Edge_noncrit_mask_path):
        #     Edge_noncrit_mask = torch.load(Edge_noncrit_mask_path, map_location="cpu")
        # else:
        #     Edge_noncrit_mask = make_edge_mask(
        #         Edge_noncrit,
        #         drop_prob=0.3,
        #         seed=12345,
        #         symmetric=True
        #     ).cpu()
        #     torch.save(Edge_noncrit_mask, Edge_noncrit_mask_path)
    ...
    def forward(self, training=True):
        if hasattr(self, 'covariate'):
            ...
            inputs = self.pred_gt
        self.mean, self.node_mask = self.netEncoder(inputs, self.adj, self.Edge_crit, training)
        sample = self.mean
        self.prediction = self.netDecoder(sample, self.adj, self.Edge_crit)
