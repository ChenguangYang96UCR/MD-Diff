...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        ## for morsediffusion
        parser.add_argument('--pretrain', type=str, default='', help='pre-traind encoder and decoder for morsediffusion model')
        # dataset parameters
        ...
        parser.add_argument('--t_len', type=int, default=24, help='time length')
    ...
    def parse(self):
        ...
        if opt.phase == 'test' or opt.continue_train:
            if opt.file_time == '':
                ...
                opt.name = opt.model + '_' + opt.pred_attr + '_' + opt.file_time
        else:
            current_time = time.strftime("%Y%m%dT%H%M%S", time.localtime())
            opt.name = opt.model + '_' + opt.pred_attr + '_' + current_time
        ...
        if opt.phase == 'test':
            ...
            if os.path.exists(yaml_path):
                ...
                print('Cannot find configuration file, use default.')
        if opt.phase == 'train' or (opt.phase == 'test' and not load_test_yaml):
