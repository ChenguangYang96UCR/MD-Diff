...
def crps_mc_loss(samples, target, mask=None):
    """
    samples: [S, T, N, D]
    target:  [T, N, D]
    mask:    [T, N, 1] or [T, N, D]  (1 = observed, 0 = missing)
    """

    target = target.unsqueeze(0)  # [1,T,N,D]

    if mask is not None:
        mask = mask.unsqueeze(0)

    # E|X - y|
    term1 = torch.abs(samples - target)
    if mask is not None:
        term1 = term1 * mask
    ...
    # E|X - X'|
    S = samples.shape[0]
    s1 = samples.unsqueeze(1)   # [S,1,T,N,D]
    s2 = samples.unsqueeze(0)   # [1,S,T,N,D]
    pairwise = torch.abs(s1 - s2)
    if mask is not None:
        pairwise = pairwise * mask.unsqueeze(1)
    term2 = pairwise.mean(dim=(0,1))
